...
- [Legend](#legend)
- [Summary](#summary)
- [1. Ujt Class (V2.0.0)](#1-ujt-class-v200)
  - [1.1 Ujt Variables](#11-ujt-variables)
    - [1.1.1 `Ujt.VERSION`](#111-ujtversion)
    - [1.1.2 `Ujt.CONTRIBUTORS`](#112-ujtcontributors)
    - [1.1.3 `Ujt.MINIMUM_JAVA_VERSION`](#113-ujtminimum_java_version)
    - [1.1.4 `Ujt.RELEASE_DATE`](#114-ujtrelease_date)
    - [1.1.5 `Ujt.BUILD`](#115-ujtbuild)
    - [1.1.6 `Ujt.LICENSE`](#116-ujtlicense)
  - [1.2 Ujt Methods](#12-ujt-methods)
    - [1.2.1 `Ujt.checkJavaVersion()`](#121-ujtcheckjavaversion)
...
- [2. Binary Class (V2.0.0)](#2-binary-class-v200)
...
- [3. ClipboardUtils Class (V2.0.0)](#3-clipboardutils-class-v200)
...
- [4. ColorUtils Class (V2.0.0)](#4-colorutils-class-v200)
...
- [5. Cryptography Class (V2.0.0)](#5-cryptography-class-v200)
...
- [6. Csv Class (V2.0.0)](#6-csv-class-v200)
...
- [7. DateUtils Class (V2.0.0)](#7-dateutils-class-v200)
...
- [8. Encoding Class (V2.0.0)](#8-encoding-class-v200)
...
- [9. FileUtils Class (V2.0.0)](#9-fileutils-class-v200)
...
- [10. HttpUtils Class (V2.0.0)](#10-httputils-class-v200)
  - [10.1 HttpUtils Variables](#101-httputils-variables)
    - [10.1.1 `HttpUtils.DEFAULT_TIMEOUT_SECONDS`](#1011-httputilsdefault_timeout_seconds)
    - [10.1.2 `HttpUtils.HttpResult`](#1012-httputilshttpresult)
...
- [11. Json Class (V2.0.0)](#11-json-class-v200)
...
- [12. Jvm Class (V2.0.0)](#12-jvm-class-v200)
...
- [13. MathUtils Class (V2.0.0)](#13-mathutils-class-v200)
...
- [14. RandomUtils Class (V2.0.0)](#14-randomutils-class-v200)
...
- [15. RegexUtils Class (V2.0.0)](#15-regexutils-class-v200)
...
- [16. StringUtils Class (V2.0.0)](#16-stringutils-class-v200)
...
- [17. SystemInfo Class (V2.0.0)](#17-systeminfo-class-v200)
...
- [18. Terminal Class (V2.0.0)](#18-terminal-class-v200)
...
- [19. Validate Class (V2.0.0)](#19-validate-class-v200)
...
- [20. ZipUtils Class (V2.0.0)](#20-ziputils-class-v200)
...
- ## **Total Methods and Variables**: 318

# 1. Ujt Class (V2.0.0)
## 1.1 Ujt Variables
### 1.1.1 `Ujt.VERSION`
- **Type**: `String`
- **Description**: The version of Useful Java Tools (`"V"` + `"Major"."Minor"."Patch"`).

### 1.1.2 `Ujt.CONTRIBUTORS`
- **Type**: `String[]`
- **Description**: The people who contributed to Useful Java Tools.

### 1.1.3 `Ujt.MINIMUM_JAVA_VERSION`
- **Type**: `int`
- **Description**: The minimum version of Java required to run Useful Java Tools.

### 1.1.4 `Ujt.RELEASE_DATE`
- **Type**: `String`
- **Description**: The release date of the current version of the library in the format `"YYYY/MM/DD"`.

### 1.1.5 `Ujt.BUILD`
- **Type**: `String`
- **Description**: The current build identifier of Useful Java Tools (`"ujt-" + VERSION + "-" + RELEASE_DATE + "-" + MINIMUM_JAVA_VERSION`).

### 1.1.6 `Ujt.LICENSE`
- **Type**: `String`
- **Description**: The license of Useful Java Tools (MIT).

## 1.2 Ujt Methods
### 1.2.1 `Ujt.checkJavaVersion()`
- **Arguments**: None
- **Returns**: `void`
- **Description**: Checks if the current Java version is supported by this library. Throws `UnsupportedOperationException` if the running JVM is older than `MINIMUM_JAVA_VERSION`.
...
# 10. HttpUtils Class (V2.0.0)
## 10.1 HttpUtils Variables
### 10.1.1 `HttpUtils.DEFAULT_TIMEOUT_SECONDS`
- **Type**: `int`
- **Description**: The default timeout in seconds (30) used by overloads that don't take an explicit timeout.

### 10.1.2 `HttpUtils.HttpResult`
- **Type**: `record` (nested in `HttpUtils`)
- **Components**:
        - `statusCode` (`int`): The HTTP status code, or `-1` if the request never produced one.
        - `headers` (`Map<String, String>`): Response headers (multiple values for the same header are joined with `, `). May be empty on failure.
        - `body` (`String`): The response body. May be empty on failure.
        - `error` (`String`): A description of any error that occurred, or `null` on success.
- **Static factories**:
        - `HttpResult.ok(int statusCode, Map<String, String> headers, String body)` — build a successful result.
        - `HttpResult.failed(String error)` — build a failure result (`statusCode = -1`, empty headers/body, given `error`).
- **Instance methods**:
        - `isSuccess()`: `boolean` — `true` when `error == null` and the status code is in the range 200-299.
- **Description**: Container returned by every network operation in `HttpUtils`. Replaces the V1.x silent-`null`/`-1`/`false` failure modes with an explicit, inspectable result.

## 10.2 HttpUtils Methods
### 10.2.1 `HttpUtils.get(String url)`
- **Arguments**:
        - `url`: **R** The URL to send a GET request to.
- **Returns**: `HttpResult` The response (status code, headers, body) on success, or a failure result with `error` populated and `statusCode == -1` on error.
- **Description**: Sends a GET request to the given URL and returns the result.
...
- **Returns**: `HttpResult` The response on success, or a failure result on error.
...
- **Returns**: `HttpResult` The response on success, or a failure result on error.
- **Description**: Sends a POST request with the given body and returns the result.
...
- **Arguments**:
        ...
        - `body`: **R** The request body to send.
- **Returns**: `HttpResult` The response on success, or a failure result on error.
- **Description**: Sends a PUT request with the given body and returns the result.
...
- **Returns**: `HttpResult` The response on success, or a failure result on error.
- **Description**: Sends a DELETE request to the given URL and returns the result.
...
- **Returns**: `HttpResult` Carries the status code and headers; the body is empty. On error, `statusCode == -1` and `error` is populated.
- **Description**: Sends a HEAD request to the given URL.

### 10.2.10 `HttpUtils.getStatusCode(String url)`
- **Arguments**:
        - `url`: **R** The URL to query.
- **Returns**: `HttpResult` Carries the status code (and any response data the underlying GET produced). On error, `statusCode == -1` and `error` is populated.
- **Description**: Sends a GET request to the given URL and returns just the response status code (read via `result.statusCode()`).

### 10.2.11 `HttpUtils.getHeaders(String url)`
- **Arguments**:
        - `url`: **R** The URL to query.
- **Returns**: `HttpResult` Carries the response headers (read via `result.headers()`). Multiple values for the same header are joined with `, `. On error, `headers` is empty and `error` is populated.
...
- **Returns**: `HttpResult` Carries the HTTP status code on completion; `body` is empty (the bytes are written to disk). On error, `statusCode == -1` and `error` is populated. Use `result.isSuccess()` to confirm a 2xx download.
...
# 13. MathUtils Class (V2.0.0)
## 13.1 MathUtils Methods
### 13.1.1 `MathUtils.round(double x)`
- **Arguments**:
        - `x`: **R** The number to round.
- **Returns**: `double` The rounded number (0 decimal places). `NaN` and `±Infinity` are returned unchanged.
- **Description**: Rounds a number to the nearest integer.

### 13.1.2 `MathUtils.round(double x, int precision)`
- **Arguments**:
        - `x`: **R** The number to round.
        - `precision`: **R** The number of decimal places to round to. Must be non-negative.
- **Returns**: `double` The rounded number. `NaN` and `±Infinity` are returned unchanged.
- **Description**: Rounds a number to a specified number of decimal places. Throws `IllegalArgumentException` if `precision` is negative.
